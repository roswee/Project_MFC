// MyData.cpp : Defines the initialization routines for the DLL.
//

#include "pch.h"
#include "framework.h"
#include "MyData.h"
#include <iostream>
#include <fstream>
#include <cstring>

#ifdef _DEBUG
#define new DEBUG_NEW
#endif



//
//TODO: If this DLL is dynamically linked against the MFC DLLs,
//		any functions exported from this DLL which call into
//		MFC must have the AFX_MANAGE_STATE macro added at the
//		very beginning of the function.
//
//		For example:
//
//		extern "C" BOOL PASCAL EXPORT ExportedFunction()
//		{
//			AFX_MANAGE_STATE(AfxGetStaticModuleState());
//			// normal function body here
//		}
//
//		It is very important that this macro appear in each
//		function, prior to any calls into MFC.  This means that
//		it must appear as the first statement within the
//		function, even before any object variable declarations
//		as their constructors may generate calls into the MFC
//		DLL.
//
//		Please see MFC Technical Notes 33 and 58 for additional
//		details.
//

// CMyDataApp

BEGIN_MESSAGE_MAP(CMyDataApp, CWinApp)
END_MESSAGE_MAP()


// CMyDataApp construction

CMyDataApp::CMyDataApp()
{
	// TODO: add construction code here,
	// Place all significant initialization in InitInstance
}


// The one and only CMyDataApp object

CMyDataApp theApp;


// CMyDataApp initialization

BOOL CMyDataApp::InitInstance()
{
	CWinApp::InitInstance();

	return TRUE;
}


MY_POINT* MY_DATA::allocTab(MY_POINT* ptab, int n)
{
    try
    {
        if (!ptab)
        {
            ptab = new MY_POINT[n];
            capacity = n;
        }
    }
    catch (std::bad_alloc)
    {
        //CExcept1App* pExp = (CExcept1App*)pExcept;
        //pExcept->PutMessage(1003);
    }

    return ptab;
}

MY_DATA::MY_DATA(int no_it)
{
    //pExcept = GetExceptPtr();
    pTab = NULL;
    Init(no_it);
}

MY_DATA::MY_DATA(const MY_DATA& ob)
{
    capacity = ob.capacity;
    last = ob.last;
    pTab = NULL;
    pTab = allocTab(pTab, capacity);
    if (pTab)
        std::copy(ob.pTab, ob.pTab + last, pTab);
}

void MY_DATA::Init(int no_it)
{
    capacity = no_it;
    last = 0;
    pTab = allocTab(pTab, capacity);
}

void MY_DATA::Push(const MY_POINT& tmp)
{
    if (last >= capacity)
    {
        int delta = last - capacity;
        int new_capacity = (delta > capacity) ? (capacity + delta) : (2 * capacity);
        int old_capacity(capacity);
        MY_POINT* newTab(NULL);
        newTab = allocTab(newTab, new_capacity);
        std::copy(pTab, pTab + old_capacity, newTab);
        delete[] pTab;
        pTab = newTab;
        //capacity = new_capacity;
    }

    pTab[last++] = tmp;
}

void MY_DATA::GetMaxMinCoords(double& max_x, double& min_x, double& max_y, double& min_y)
{
    max_x = min_x = max_y = min_y = 0;
    if (pTab && last)
    {
        max_x = min_x = pTab[0].x;
        max_y = min_y = pTab[0].y;

        for (int i = 1; i < last; ++i)
        {
            if (pTab[i].x > max_x)
                max_x = pTab[i].x;
            if (pTab[i].y > max_y)
                max_y = pTab[i].y;
            if (pTab[i].x < min_x)
                min_x = pTab[i].x;
            if (pTab[i].y < min_y)
                min_y = pTab[i].y;
        }
    }
}

void MY_DATA::clear_all() {
    Free();
    last = 0;
    capacity = 0;
}

void MY_DATA::SaveToFile(const char* filename)
{
    std::ofstream out(filename, std::ios::binary);
    if (out.is_open())
    {
        out.write(reinterpret_cast<char*>(&capacity), sizeof(capacity));
        out.write(reinterpret_cast<char*>(&last), sizeof(last));
        for (int i = 0; i < last; ++i)
        {
            int name_len = strlen(pTab[i].name) + 1;
            out.write(reinterpret_cast<char*>(&name_len), sizeof(name_len));
            out.write(pTab[i].name, name_len);
            out.write(reinterpret_cast<char*>(&pTab[i].x), sizeof(pTab[i].x));
            out.write(reinterpret_cast<char*>(&pTab[i].y), sizeof(pTab[i].y));
            out.write(reinterpret_cast<char*>(&pTab[i].numb), sizeof(pTab[i].numb));
            out.write(reinterpret_cast<char*>(&pTab[i].color), sizeof(pTab[i].color));
        }
        out.close();
    }
}

void MY_DATA::LoadFromFile(const char* filename)
{
    std::ifstream in(filename, std::ios::binary);
    if (in.is_open())
    {
        Free();
        in.read(reinterpret_cast<char*>(&capacity), sizeof(capacity));
        in.read(reinterpret_cast<char*>(&last), sizeof(last));
        pTab = allocTab(pTab, capacity);
        for (int i = 0; i < last; ++i)
        {
            int name_len;
            in.read(reinterpret_cast<char*>(&name_len), sizeof(name_len));
            pTab[i].name = new char[name_len];
            in.read(pTab[i].name, name_len);
            in.read(reinterpret_cast<char*>(&pTab[i].x), sizeof(pTab[i].x));
            in.read(reinterpret_cast<char*>(&pTab[i].y), sizeof(pTab[i].y));
            in.read(reinterpret_cast<char*>(&pTab[i].numb), sizeof(pTab[i].numb));
            in.read(reinterpret_cast<char*>(&pTab[i].color), sizeof(pTab[i].color));
        }
        in.close();
    }
}

MY_POINT& MY_DATA::operator[](const int i) {
    return pTab[i];
}


