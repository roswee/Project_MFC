...
void MyExcept1::HandleError(const CString& errorMessage)
{
	LogError(errorMessage);
	AfxMessageBox(errorMessage);
}

void MyExcept1::LogError(const CString& errorMessage)
{
	std::ofstream logFile("my_except_error_log.txt", std::ios::app);
	if (logFile.is_open())
	{
		logFile << CT2A(errorMessage.GetString()) << std::endl;
		logFile.close();
	}
}

void CExcept1App::LogError(const CString& errorMessage)
{
...
void CExcept1App::DisplayErrorMessage(const CString& errorMessage)
{
	AfxMessageBox(errorMessage);
}
